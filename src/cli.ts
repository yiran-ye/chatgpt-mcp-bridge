...
常用选项:
  ...
  --access <mode>             read-only、workspace-write、command-exec 或 full-access
  --config <path>             command-exec 命令配置文件
...
export function parseArgs(argv: string[]): CliConfig {
  ...
  if (parsed.data.access === 'full-access' && parsed.data.configPath !== undefined) throw new AppError('INVALID_INPUT', 'full-access 不使用 --config');
  return parsed.data;
}
...
async function main(): Promise<void> {
  ...
  const needsConfigResolution = config.access !== 'full-access' && (config.command !== 'serve' || config.access === 'command-exec');
  ...
  if (config.command === 'config') { process.stdout.write(`${JSON.stringify(publicConfig(config, workspace.name, configResolution, commandConfig ? Object.keys(commandConfig.commands) : []), null, 2)}\n`); return; }
  if (config.command === 'doctor') { process.stdout.write(`${JSON.stringify(await doctor(config, workspace, configResolution, commandConfig ? Object.keys(commandConfig.commands) : []), null, 2)}\n`); return; }
...
function configDiagnostics(config: CliConfig, resolution: CommandConfigResolution | undefined): { configPath: string | null; configSource: CommandConfigResolution['source'] | null; configExists: boolean | null } { return config.access === 'full-access' ? { configPath: null, configSource: null, configExists: null } : { configPath: resolution?.path ?? null, configSource: resolution?.source ?? null, configExists: resolution?.exists ?? null }; }
function publicConfig(config: CliConfig, workspaceName: string, resolution: CommandConfigResolution | undefined, commandIds: string[]): Record<string, unknown> { const commandPolicy = config.access === 'full-access' ? 'unrestricted' : config.access === 'command-exec' ? 'allowlist' : 'none'; return { workspaceName, transport: config.transport, host: config.host, port: config.port, mcpPath: config.mcpPath, access: config.access, commandPolicy, ...configDiagnostics(config, resolution), commandIds, authenticationConfigured: Boolean(config.authToken), allowPublicBind: config.allowPublicBind, sensitiveFilesAllowed: false, readOnly: config.access === 'read-only' }; }
async function doctor(config: CliConfig, workspace: WorkspaceContext, resolution: CommandConfigResolution | undefined, commandIds: string[]): Promise<Record<string, unknown>> { const git = new GitService(workspace); const repository = await git.isRepository(); let branch: string | null = null; let head: string | null = null; if (repository) { const status = await git.status(); branch = status.branch; head = status.head; } const safeHost = ['127.0.0.1', 'localhost', '::1'].includes(config.host); const writableHttpValid = config.transport !== 'http' || config.access === 'read-only' || (safeHost && Boolean(config.authToken && config.authToken.length >= 32)); const commandPolicy = config.access === 'full-access' ? 'unrestricted' : config.access === 'command-exec' ? 'allowlist' : 'none'; return { ok: Number(process.versions.node.split('.')[0]) >= 20 && repository && writableHttpValid, node: { version: process.versions.node, supported: Number(process.versions.node.split('.')[0]) >= 20 }, git: { available: await executableAvailable('git'), repository, rootName: repository ? basename(workspace.root) : null, branch, head }, ripgrep: { available: await executableAvailable('rg') }, workspace: { exists: true, name: workspace.name }, access: { mode: config.access, commandPolicy, ...configDiagnostics(config, resolution), commandIds }, ignoreFile: { exists: await fileExists(`${workspace.root}/.chatgpt-mcp-bridge-ignore`) }, http: { host: config.host, safeHost, port: config.port, portAvailable: await portAvailable(config.host, config.port), publicBindAllowed: config.allowPublicBind, authConfigured: Boolean(config.authToken), writableConfigurationValid: writableHttpValid, dangerousPublicConfiguration: !safeHost && (!config.allowPublicBind || !config.authToken) }, sensitiveFilesAllowed: false } }
