...
export function registerTools(server: McpServer, workspace: WorkspaceContext, logger?: AuditLogger, options?: BridgeOptions): void {
  ...
  const wrap = (name: string, action: (input: never) => Promise<Record<string, unknown>>) => async (input: never): Promise<ToolResult> => {
    ...
    try { const data = await action(input); const text = JSON.stringify(data, null, 2); await logger?.event(name, 'allow', Date.now() - started, Buffer.byteLength(text), undefined, target); return { content: [{ type: 'text', text }], structuredContent: { data } }; }
