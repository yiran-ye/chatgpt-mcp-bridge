...
export async function createFixture(): Promise<GitFixture> {
  const parent = await mkdtemp(path.join(tmpdir(), 'local-code-mcp-test-')); const root = path.join(parent, 'repo'); const outside = path.join(parent, 'outside.txt'); await mkdir(root); await writeFile(outside, 'outside secret\n');
