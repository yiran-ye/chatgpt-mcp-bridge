...
直接写入的 patch 上限 256 KiB、结果文件上限 1 MiB。命令默认使用目录配置的超时和输出上限，硬超时 10 分钟、stdout/stderr 合计硬上限 1 MiB；附加参数最多 64 个且单项最多 4096 字节。命令配置最大 64 KiB，未知字段会被拒绝。项目级 `.chatgpt-mcp-bridge/config.json` 只允许 Bridge 控制面加载，MCP 文件读取与写入工具不能访问或修改它。
