...
本项目默认安全边界是“固定 workspace 内、策略允许的只读数据”。`workspace-write` 与 `command-exec` 是启动时显式启用的高权限模式，MCP 调用不能切换模式。
...
- `run_command` 只能选择用户级配置中的命令 ID。模型不能覆盖 executable、固定参数、环境变量、超时或输出上限。
- 命令执行不是系统沙箱。被允许的程序具有当前 OS 用户的权限和潜在网络能力，也可能间接访问受保护路径；`command-exec` 与宿主的免审批模式只能用于可信工作区。

## HTTP

默认只监听 `127.0.0.1`。loopback 模式为了兼容 MCP Client，允许缺失 Origin；若提供 Origin，则只接受 loopback Origin。只读模式的 public bind 必须同时设置 `--allow-public-bind` 和 Bearer Token。写入和命令模式禁止 public bind，且 HTTP loopback 也要求至少 32 字符的 Bearer Token。服务不配置任意 CORS。
