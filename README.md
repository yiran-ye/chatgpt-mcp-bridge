...
> 将 ChatGPT 安全连接到本地 Git 工作区的只读 MCP Bridge。

`chatgpt-mcp-bridge` 是一个独立、只读的本地代码 MCP Server。它把**单个**本地 Git 工作区的状态、diff、源码和项目指令安全地提供给 ChatGPT、MCP Inspector 或其他 MCP Client，用于 Code Review 与架构分析。启动参数决定当前连接哪个项目；同一个通用 Tunnel 可以在切换 workspace 后继续复用。
...
不要用无认证隧道或把本地端口裸露到公网。非 loopback bind 必须显式提供 `--allow-public-bind`，并通过 `CHATGPT_MCP_BRIDGE_TOKEN` 或 `--auth-token` 配置 Bearer Token。
